...
IGNORED_CAPTION_APPS = frozenset((
    'com.mbp-intel.dropdown', 'mbp-intel-dropdown', 'com.mbp-intel.monitor',
))
...
def action_command(button, home=Path.home(), shifted=False):
    ...
    if button == 1:
        return [str(home / '.local/bin/mbp-intel-control'), 'windows']
    if button == 2:
        return ['swaymsg', 'floating', 'toggle']
    if button == 3 and shifted:
        return [str(home / '.local/bin/mbp-intel-decoration-settings')]
    return None
