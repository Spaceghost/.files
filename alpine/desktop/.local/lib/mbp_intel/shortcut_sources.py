...
``~/.config/mbp-intel/shortcuts.json``. Its schema is::
...
class ShortcutProvider:
    ...
    def __init__(self, socket_path, profiles_path=None):
        ...
        if profiles_path is None:
            profiles_path = Path.home() / '.config/mbp-intel/shortcuts.json'
    ...
    def _sway_description(command):
        ...
        if re.fullmatch(r'exec (?:[^ ]*/)?mbp-intel-carousel cancel, mode ["\']?default["\']?', command):
            return 'Leave the overview and return to your window'
        if re.fullmatch(r'exec (?:[^ ]*/)?mbp-intel-carousel cancel, mode ["\']?default["\']?, exec (?:[^ ]*/)?mbp-intel-menu', command):
            return 'Leave the overview and open the application launcher'
        if re.fullmatch(r'mode ["\']?default["\']?, exec (?:[^ ]*/)?mbp-intel-menu', command):
            return 'Leave the overview and open the application launcher'
        if command == 'kill':
        ...
        if command.startswith('exec '):
            ...
            if arguments:
                ...
                known_exec = {
                    ('foot',): 'Open terminal',
                    ('mbp-intel-dropdown',): 'Toggle drop-down console',
                    ('mbp-intel-dropdown', 'monitor'): 'Toggle system monitor',
                    ('mbp-intel-center',): 'Center and raise focused window',
                    ('mbp-intel-resize', 'near-full'): 'Give this window room: float, nearly fill, and center',
                    ('mbp-intel-carousel', 'next', '--modifier', 'super'): 'Next recent window — hold Super to browse, release to land',
                    ('mbp-intel-carousel', 'previous', '--modifier', 'super'): 'Previous window — Shift reverses; Escape brings you back',
                    ('mbp-intel-carousel', 'next', '--modifier', 'alt'): 'Next recent window — hold Alt to browse, release to land',
                    ('mbp-intel-carousel', 'previous', '--modifier', 'alt'): 'Previous window — Shift reverses; Escape brings you back',
                    ('mbp-intel-carousel', 'show'): 'Browse every workspace: choose a window, then Enter to land',
                    ('mbp-intel-showdesktop', 'restore-or-carousel'): 'Bring hidden windows home, or browse every window',
                    ('mbp-intel-control',): 'Open system controls',
                    ('mbp-intel-lock',): 'Lock session',
                    ('mbp-intel-menu',): 'Open application launcher',
                    ('mbp-intel-wallpaper', 'pick'): 'Choose wallpaper',
                    ('mbp-intel-wallpaper', 'next'): 'Next wallpaper',
                    ('mbp-intel-wallpaper', 'prev'): 'Previous wallpaper',
                    ('mbp-intel-wallpaper', 'pause'): 'Pause or resume wallpaper rotation',
                    ('mbp-intel-workspaces', 'ai-next'): 'Focus next AI workspace',
                    ('mbp-intel-workspaces', 'ai-menu'): 'Choose AI workspace',
                    ('swaync-client', '-t'): 'Toggle notification center',
                    ('mbp-intel-screenshot', 'full'): 'Capture full screen',
                    ('mbp-intel-screenshot', 'area'): 'Capture selected area',
                    ('mbp-intel-screenshot', 'window'): 'Capture focused window',
                    ('mbp-intel-audio', 'up'): 'Increase volume',
                    ('mbp-intel-audio', 'down'): 'Decrease volume',
                    ('mbp-intel-audio', 'mute'): 'Toggle audio mute',
