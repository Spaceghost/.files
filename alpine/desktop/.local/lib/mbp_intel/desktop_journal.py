...
def main():
    ...
    parser.add_argument('--database', type=Path, default=data / 'mbp-intel/journal/entries.sqlite3')
    ...
    with closing(open_database(args.database)) as db:
        if not db.execute("SELECT 1 FROM metadata WHERE key='seed-v1'").fetchone():
            source = Path(__file__).resolve().parents[2] / 'share/mbp-intel/journal-seed.json'
