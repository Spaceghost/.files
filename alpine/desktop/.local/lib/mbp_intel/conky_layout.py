...
def render_config(panel, placement, colours, options):
    ...
    settings = {
        ...
        'own_window_class': lua_string('mbp-intel-conky'),
        'own_window_title': lua_string('mbp-intel-conky-' + panel['id']),
    ...
    if options.get('click_hook') and panel['id'] in ('scripture', 'witness', 'ghost', 'gallery'):
        ...
        settings['lua_mouse_hook'] = lua_string('mbp_intel_click')
    ...
    if panel['id'] == 'scripture' and 'SCRIPTURE' in heading and options.get('click_hook'):
        ...
        settings['lua_startup_hook'] = lua_string(f'mbp_intel_history {left} {width} {height}')
    for index, colour in enumerate(colours['series'][:6], start=3):
        settings[f'color{index}'] = lua_string(colour)
    lines = ['-- Generated by mbp-intel-conky; edits are replaced on the next wallpaper change.',
             'conky.config = {']
