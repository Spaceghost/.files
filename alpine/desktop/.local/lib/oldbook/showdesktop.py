...
def private_directory(runtime):
    path = runtime / 'oldbook' / 'showdesktop'
...
def serve(ipc, sway, directory, control):
    ...
    def gesture(fd, condition):
        ...
        # A swipe that lands mid-animation is dropped; the running pass owns
        # the screen until it hands the workspace back.
        if not active['gesture'] and action in ('show', 'restore', 'toggle'):
            active['gesture'] = True
            try:
                perform(action, ipc, sway, directory)
            except (KeyError, OSError, ValueError, RuntimeError,
                    subprocess.CalledProcessError) as error:
                print(f'oldbook-showdesktop: {error}', file=sys.stderr, flush=True)
            finally:
                active['gesture'] = False
        return GLib.SOURCE_CONTINUE

    def ended(fd, condition):
        loop.quit()
        return GLib.SOURCE_REMOVE

    subscription = socket.socket(socket.AF_UNIX)
    subscription.connect(str(sway))
    ipc['send'](subscription, 2, '["shutdown"]')
    _, accepted = ipc['receive'](subscription)
    if not accepted.get('success'):
        raise RuntimeError('Sway rejected the shutdown subscription')
    with subscription:
        GLib.unix_fd_add_full(GLib.PRIORITY_DEFAULT, control.fileno(),
                              GLib.IOCondition.IN, gesture)
        GLib.unix_fd_add_full(GLib.PRIORITY_DEFAULT, subscription.fileno(),
                              GLib.IOCondition.IN | GLib.IOCondition.HUP, ended)
        loop.run()
...
def run(action):
    ...
    if deliver(control_address(directory, sway), action):
        return
    # No helper is listening; do the work here rather than ignore the swipe.
    perform(action, load_ipc(), sway, directory)


def perform(action, ipc, sway, directory):
    with (directory / 'gesture.lock').open('a') as lock:
        ...
        if state and not hidden:
            # The workspace was changed by hand while the desktop was bare.
        ...
        if action == 'restore':
            if state and hidden:
                animate(ipc, sway, directory, state, restoring=True)
            return
        if hidden or state:
            return
        ...
        animate(ipc, sway, directory, plan, restoring=False)
...
def animate(ipc, sway, directory, state, restoring):
    preload_layer_shell()
    import gi
    gi.require_version('Gtk', '4.0')
    ...
    Gtk4LayerShell.set_namespace(window, 'oldbook-showdesktop')
    ...
    def swap(command):
        try:
            ipc['command'](sway, command)
        except (OSError, RuntimeError, ValueError):
            # Losing the swap is recoverable: the overlay still tears down.
            pass
    ...
    def tick(widget, clock):
        ...
        if phase == 'settle':
            ...
            if not presented and now < schedule['deadline']:
                return GLib.SOURCE_CONTINUE
            if not restoring:
                swap('workspace ' + json.dumps(HIDDEN_WORKSPACE))
    ...
    loop.run()
    # The captures are tens of megabytes. The tick callback and the child widget
    # both hold the closure that owns them, so an overlay left attached would
    # keep a whole screen of pixels alive for every gesture of the session.
...
def main(action):
    ...
    try:
        ...
        sys.exit(f'oldbook-showdesktop: {error}')
