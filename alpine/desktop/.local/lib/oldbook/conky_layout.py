...
CENTRE_PENALTY = 1.4
MIN_CONTRAST = 4.5
# Cards that answer the pointer. Applications keep their own input above them.
CLICK_CARDS = ('scripture', 'witness', 'ghost', 'gallery', 'rice')
...
def header_link(heading, placement, options, label):
    """Reserve a right-aligned text link on a card's heading line.

    The hit area is measured here because the click hook runs in its own Lua
    state: geometry only reaches it through the startup hook the caller writes.
    """
    size = re.search(r'(?:^|:)size=([0-9.]+)', options.get('font', ''))
    points = float(size[1]) if size else 9
    width, height = math.ceil(points * (len(label) + 1)), math.ceil(points * 2)
    left = max(0, placement['width'] - width)
    heading = heading.split('${hr', 1)[0].rstrip()
    return (heading + f'${{goto {left}}}${{color1}}{label}${{color}}', left, width, height)
...
def render_config(panel, placement, colours, options):
    ...
    if options.get('click_hook') and panel['id'] in CLICK_CARDS:
    ...
    if panel['id'] == 'scripture' and 'SCRIPTURE' in heading and options.get('click_hook'):
        ...
        heading, left, width, height = header_link(heading, placement, options, 'History')
        text = heading + separator + body
    ...
    elif panel['id'] == 'rice' and options.get('click_hook'):
        # The same target on the rice card, running the selected entry rather
        # than opening a reader. Conky has no hover, so the card's own body
        # carries the description and the link only has to be reachable.
        heading, left, width, height = header_link(heading, placement, options, 'Try')
        text = heading + separator + body
        settings['lua_startup_hook'] = lua_string(f'oldbook_rice {left} {width} {height}')
