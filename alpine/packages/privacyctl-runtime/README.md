# Oldbook radio runtime APK

`oldbook-radio-runtime` installs only the fixed privileged CLI, fourteen Python
...
alpine/security/radio/make-runtime-release \
  --output /tmp/oldbook-radio-runtime-0.1.0.tar.gz
```
...
alpine/packages/privacyctl-runtime/build-offline \
  --source /tmp/oldbook-radio-runtime-0.1.0.tar.gz --work /tmp/radio-build-one
alpine/packages/privacyctl-runtime/build-offline \
  --source /tmp/oldbook-radio-runtime-0.1.0.tar.gz --work /tmp/radio-build-two
```
