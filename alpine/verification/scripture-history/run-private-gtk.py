#!/usr/bin/env python3
"""Run the synthetic Scripture history reader fixture in a private Xvfb session."""
...
with tempfile.TemporaryDirectory(prefix='scripture-history-gtk-') as directory:
    base = Path(directory)
    env = dict(os.environ)
    ...
    with (args.output / 'xvfb.log').open('w') as server_log:
        server = subprocess.Popen(['Xvfb', '-displayfd', str(write_fd), '-nolisten', 'tcp', '-screen', '0', '1200x900x24', '-ac'], env=env, pass_fds=(write_fd,), stdin=subprocess.DEVNULL, stdout=server_log, stderr=server_log, start_new_session=True)
        os.close(write_fd)
        try:
