...
with tempfile.TemporaryDirectory(prefix='expo-check-') as temp:
    ...
    with (base/'run.log').open('w') as log:
        ...
        try:
            ...
            keyboard=subprocess.Popen(['wtype','-s','2000','Workspace 2','-s','300','-k','Return','-s','2500','Terminal','-s','300','-k','Return','-s','30000'],env=env,stdout=log,stderr=log);processes.append(keyboard)
            time.sleep(.2)
            expo=launch()
            ...
            expo.wait(timeout=5)
            print(json.dumps({'search_focuses_window':True,'escape_cancels':True,'toggle_closes':True,'show_is_idempotent':True,'fuzzel_picker':True,'overview_rendered' :True,'keyboard_workspace_selection':True,'close_command':True,'gesture_config_loaded':True}))
        except Exception:
