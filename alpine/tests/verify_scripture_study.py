...
def native_verification(root, log):
    ...
    try:
        ...
        selection_path = home / '.local/state/oldbook/scripture/selection.json'
        ...
        scripture_config.write_text(
            ...
            "own_window_class='oldbook-conky',own_window_title='oldbook-conky-scripture',"
        ...
        try:
            pids = json.loads((home / '.local/state/oldbook/conky/pids.json').read_text())
        except (OSError, ValueError):
            pids = {}
...
def live_activation_observation():
    ...
    live_database = Path('/home/jack/.local/share/oldbook/scripture/study.sqlite3')
    archived_database = Path(
        '/home/jack/.local/share/oldbook/scripture/'
        'study-before-application-id-1788838969347774079.sqlite3')
    selection_backup = Path(
        '/home/jack/.local/state/oldbook/scripture/'
        'selection-before-study-1788838996144792109.json')
    preview_journal = Path(
        '/tmp/oldbook-scripture-study-preview/.local/state/oldbook/backups/'
        '1788838969793306374')
    live_journal = Path(
        '/home/jack/.local/state/oldbook/backups/1788838970027102079')
    selection = json.loads(Path(
        '/home/jack/.local/state/oldbook/scripture/selection.json').read_text())
    ...
    bar_pid = int(Path('/home/jack/.local/state/oldbook/scripture/bar.pid').read_text())
    ...
    assert bar_command.endswith('/home/jack/.local/bin/oldbook-scripture-bar')
    return {
