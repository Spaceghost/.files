...
''')
    ...
    def test_right_click_returns_to_the_passage_shown_before(self):
        ...
        script.write_text('conky_config = "/tmp/scripture.conf"\n'
                          + f'dofile({json.dumps(str(hook))})\n'
                          + "assert(conky_oldbook_click({type='button_down',button='right',x=60,y=50}) == true)\n")
        result = subprocess.run(['lua', str(script)], env=self.env,
                                capture_output=True, text=True, timeout=10)
        self.assertEqual(result.returncode, 0, result.stderr)
        deadline = time.monotonic() + 10
        ...
        listing = self.command('oldbook-scripture', 'history-list').splitlines()
...
local function press_right(x, y)
    assert(conky_oldbook_click({type='button_down', button='right', x=x, y=y}))
    assert(commands[#commands]:match('oldbook%-conky%-click" scripture%-previous >'),
...
assert(#commands == 9)
conky_config = '/tmp/witness.conf'
assert(conky_oldbook_click({type='button_down', button='right', x=30, y=40}) == false)
assert(#commands == 9)
''')
        result = subprocess.run(['lua', str(script)], env=self.env,
                                capture_output=True, text=True, timeout=10)
