...
def verify(output, scales):
    ...
    report = {'scenarios': [], 'isolation': 'private HOME, XDG data, runtime, compositor',
              'sources': {str(path.relative_to(REPO)): hashlib.sha256(path.read_bytes()).hexdigest()
                for path in [BIN / 'mbp-intel-scripture-bar', BIN / 'mbp-intel-scripture',
                    REPO / 'alpine/desktop/.local/lib/mbp_intel/scripture_search.py',
                    REPO / 'alpine/desktop/.local/lib/mbp_intel/scripture_bar_ipc.py',
    ...
    with tempfile.TemporaryDirectory(prefix='scripture-inline-') as temporary:
        ...
        config.write_text('output HEADLESS-1 mode 1440x900\noutput * bg #202024 solid_color\n'
            ...
            f'bindsym Mod4+slash exec {BIN}/mbp-intel-scripture find\n'
            f'bindsym Mod4+question exec {BIN}/mbp-intel-scripture find --all\n')
        ...
        with (output / 'native.log').open('w') as log:
            ...
            def surfaces():
                return [surface for item in outputs() for surface in item.get('layer_shell_surfaces', [])
                        if surface.get('namespace') == 'mbp-intel-scripture']
            def normal_windows():
                def walk(node):
                    yield node
            ...
            try:
                ...
                for identifier in ('scripture', 'witness'):
                    text = ('${execpi 3600 ' + str(BIN / 'mbp-intel-scripture') + ' panel --width 52}'
                ...
                daemon = spawn([sys.executable, '-c', BAR_OBSERVER, str(BIN / 'mbp-intel-scripture-bar'),
                                str(root / 'bar.json')])
                ...
                for scale in scales:
                    ...
                    saved_path = home / '.local/state/mbp-intel/scripture/selection.json'
                    ...
                    history = json.loads(command(str(BIN / 'mbp-intel-scripture'), 'history-list', '--json', '--limit', '1'))
                    ...
                    activators = [subprocess.Popen([str(BIN / 'mbp-intel-scripture'), 'find'], env=env,
