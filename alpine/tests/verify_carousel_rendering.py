...
LIBRARY = Path(os.environ.get('MBP_INTEL_RENDER_LIBRARY', ROOT / 'alpine/desktop/.local/lib/mbp_intel'))
BUS_MARKER = 'MBP_INTEL_CAROUSEL_RENDER_BUS'
...
def popup_probe(output, compose_cards, warm):
    ...
    view.window.realize()
    gate = Path(os.environ['MBP_INTEL_RENDER_PRIORITY_GATE'])
...
def private_compositor(output, scale, compose_cards, warm):
    ...
    with (output / 'runtime.log').open('w') as log:
        try:
            ...
            env['MBP_INTEL_RENDER_PRIORITY_GATE'] = str(gate)
...
helper = runpy.run_path('/usr/local/sbin/mbp-intel-ui-priority')
threads = []
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='carousel-render-') as directory:
        env = dict(os.environ)
        env['MBP_INTEL_RENDER_LIBRARY'] = str(LIBRARY)
