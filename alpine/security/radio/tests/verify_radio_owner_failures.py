...
The production runner/guardian, CLI, IPC, DHCP client and lease applier are real.
WPA and radio hardware are substitutes. A real contained command pauses before
resolver publication, with the production command/check/deadline path intact.
"""
import argparse
import hashlib
import json
...
# Executed ONLY through NativeNetwork's unchanged contained-command launcher.
...
def guardian(host, expected, invocation):
    base.fixture_guard(host, expected)
    ...
    from privacyctl_runtime import runner
    from privacyctl_runtime.adapter import NativeAdapter, JournaledApplier
    ...
    class FixtureAdapter(NativeAdapter):
        ...
        def applier(self, generation, check):
            return JournaledApplier(LeaseApplier(generation, backend=PausedNetwork(), check=check),
...
def worker(host):
    # Same independently reviewed pre-mutation guards as the existing7case test.
    ...
    def start_guardian():
        ...
        process, handle = spawn([PYTHON, '-I', str(SCRIPT), '--guardian', json.dumps(host),
                                json.dumps(expected), invocation], 'guardian-' + invocation)
        base.wait_for(lambda: state(invocation).get('phase') == 'idle', 4,
    ...
    def cli(name, *arguments):
        return spawn([CLI, *arguments], name)
    ...
    try:
        ...
        for kill_requester in (False, True):
            ...
            if kill_requester:
                ...
                finish_cli(off, True)
                finish_cli(request, False)
            base.wait_for(lambda: clean(invocation), 5, 'WPA cancellation cleanup', (guard_process,))
        ...
        for kill_hook in (False, True):
            ...
            ready = base.wait_for(lambda: (row if (row := read_json(FAULTS / 'pause-ready.json', {}))
                                 ...
                                 7, 'real contained application command readiness', (guard_process, request))
            ...
            if kill_hook:
                ...
                finish_cli(off, True)
            finish_cli(request, False)
            base.wait_for(lambda: clean(invocation), 5, 'blocked hook/application cleanup', (guard_process,))
        ...
        other, _ = spawn([PYTHON, '-I', str(SCRIPT), '--guardian', json.dumps(host),
                          json.dumps(expected), other_invocation], 'second-guardian')
        ...
        owner_handle.wait(timeout=8)
        for handle in dhcp_handles:
            handle.wait()
        base.wait_for(network_clean, 4, 'guardian death cleanup')
        ...
        replacement, replacement_process, _, _ = start_guardian()
        request, _ = cli('orphan-restart-refused', 'connect', base.HOME_PROFILE)
        finish_cli(request, False)
        base.wait_for(lambda: state(replacement).get('phase') == 'idle', 3,
                      'orphaned generation refusal', (replacement_process,))
        assert not state(replacement)['client']
        assert 'orphaned lease state requires recovery' in read_tail(FAULTS / 'orphan-restart-refused.log', 8192)
        assert (RUNTIME / 'lease-dirty').read_text() == generation + '\n'
        assert read_json(PRIVATE / 'radios.json')['wifi_blocked']
        record('owner_sigkill_guardian_block_and_orphan_restart_refusal', started,
               previous_dhcp_dead=True, replacement_client_started=False, orphan_marker_preserved=True)
        ...
        base.emit({'phase': 'complete', 'cases': cases, 'guardian_is_production': True,
                   'existing_seven_case_harness_changed': False})
    except BaseException:
...
def main():
    ...
    started = time.monotonic()
    result = subprocess.run([*privilege, '/usr/bin/timeout', '-k', '3', '120', '/usr/bin/unshare',
        '--net', '--mount', '--pid', '--fork', '--kill-child=KILL', '--propagation', 'unchanged',
        PYTHON, '-I', str(SCRIPT), '--worker', json.dumps(before['namespaces'])],
        capture_output=True, text=True, timeout=128)
    ...
    evidence = {'schema_version': 1, 'status': 'passed' if passed else 'failed',
        ...
        'limitations': ['WPA is a synthetic Unix-datagram responder; rfkill is simulated by a private veth.',
            'The native-apply pause is a fixed synthetic command run through the unchanged real contained runner.',
            'No physical radio, real trusted association, packet gate, OpenSnitch or OpenRC startup is exercised.',
            'Owner SIGKILL intentionally leaves a private orphan marker and lease state; replacement authorization must refuse it.']}
