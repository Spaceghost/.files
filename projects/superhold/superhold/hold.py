"""Portable hold state and read-only Linux input monitoring."""
import errno
import fcntl
import os
import struct
...
class HoldState:
    ...
    def __init__(self, trigger='super', threshold=0.5):
        from .config import Config
        config = Config(trigger=trigger, hold_seconds=threshold)
        self.trigger_keys = (frozenset((KEY_CAPSLOCK,)) if trigger == 'capslock'
                             else SUPER_KEYS)
        self.threshold = config.hold_seconds
        self._pressed = {}
    ...
    def update(self, device, code, value, now):
        ...
        if code not in self.trigger_keys:
            if self._super_down():
                self.cancel()
            return
        ...
        super_down = {(item_device, item_code)
                      ...
                      if item_code in self.trigger_keys}
