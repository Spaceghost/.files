...
``$XDG_CONFIG_HOME/superhold/profiles.json``. Its schema is::
...
_TERMINALS = {'foot', 'footclient', 'qterminal', 'xterm', 'uxterm', 'konsole',
...
class ShortcutProvider:
    ...
    def snapshot(self):
        ...
        window, output = self._context()
        return self._snapshot_for_context(window, output, index, profile_error)
    ...
    def _terminal_identity(self, window, terminal_source):
        ...
        foreground_ttys = {
            process['tty'] for child_pid, process in processes.items()
            if (child_pid in descendants and child_pid != pid
                and process.get('tty') and process.get('tpgid', -1) > 0
                and process.get('pgrp') == process.get('tpgid'))
        }
        if len(foreground_ttys) > 1:
            # Shared-process terminals expose one WM PID for several windows
            # or tabs. Their descendant PTYs cannot identify the focused tab.
            return terminal_source, None
