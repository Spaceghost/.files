...
def main():
    ...
    parser.add_argument('--library', type=Path,
                        help='built libsuperhold-layer-shell.so; otherwise use installed lookup')
...
''')
    ...
    processes = []
    try:
        with tempfile.TemporaryDirectory(prefix='superhold-wayland-') as runtime:
            env = os.environ.copy()
            ...
            with (output / 'sway.log').open('w') as log, (output / 'qt.log').open('w') as qt_log:
                try:
                    ...
                    foot = subprocess.Popen([
                        binaries['foot'], '--app-id=superhold-focus-proof', '--config',
                    ...
                    if library:
                        qt_env['SUPERHOLD_LAYER_SHELL_LIBRARY'] = str(library)
                    if args.qt_theme:
                    ...
                    for delay, name in ((1.0, 'first-output'), (1.3, 'second-output'),
                        ...
                        if focused != ['superhold-focus-proof']:
                    ...
                    if not all(', 3, "superhold")' in line for line in surfaces):
