...
def child(output):
    ...
    from superhold.hold import KEY_CAPSLOCK, KEY_LEFTMETA
    from superhold.service import SessionLease
    from superhold.x11 import physical_trigger_codes
    ...
    control.set_wm_name('Superhold synthetic application')
    control.set_wm_class('superhold-test', 'firefox')
    ...
    config_dir = config_home / 'superhold'
    config_dir.mkdir()
    ...
    for trigger, raw in [('super', physical[KEY_LEFTMETA]), ('capslock', physical[KEY_CAPSLOCK])]:
        ...
        command = [sys.executable, '-m', 'superhold']
...
def main():
    ...
    if args.child:
        child(output)
        return
    with tempfile.TemporaryDirectory(prefix='superhold-daemon-x11-') as directory:
        ...
        try:
            ...
            try:
                ...
                stalled = subprocess.run([sys.executable, '-m', 'superhold', 'dump', '--backend', 'x11'],
