...
class QtOverlayTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        try:
            cls.module = importlib.import_module('superhold.qt_overlay')
    ...
    def test_installed_native_library_resolves_under_destdir_prefix(self):
        with tempfile.TemporaryDirectory(prefix='help-install-') as directory:
            ...
            module = prefix / 'share/superhold/superhold/qt_overlay.py'
            library = prefix / 'lib64/superhold/libsuperhold-layer-shell.so'
            ...
            source = prefix / 'projects/superhold/superhold/qt_overlay.py'
            self.assertNotEqual(self.module._default_layer_library(source), library)

    def test_legacy_library_override_and_symbols_remain_compatible(self):
        prepare = lambda: 0
        configure = lambda *args: 0
        legacy = SimpleNamespace(hth_layer_shell_prepare=prepare,
                                 hth_layer_shell_configure=configure)
        with patch.dict(os.environ, {'HOLD_TO_HELP_LAYER_SHELL_LIBRARY': '/legacy/bridge.so'}, clear=True), \
                patch.object(self.module, '_layer_bridge', None), \
                patch.object(self.module.ctypes, 'CDLL', return_value=legacy) as load:
            try:
                bridge = self.module._load_layer_bridge()
            except RuntimeError as error:
                self.fail(f'legacy override and ABI must remain usable: {error}')
            self.assertEqual(load.call_args.args, ('/legacy/bridge.so',))
            self.assertEqual(bridge.superhold_layer_shell_prepare(), 0)
            self.assertEqual(bridge.superhold_layer_shell_configure(None, None, 1, 1, 0), 0)
