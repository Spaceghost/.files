...
try:
    from superhold.service import (AlreadyRunning, GraphicalSessionGuard,
...
class RuntimeSafetyTests(unittest.TestCase):
    ...
    def test_status_command_uses_package_imports_without_loading_gtk(self):
        ...
        result = subprocess.run(
            [sys.executable, '-m', 'superhold', 'status', '--socket', str(self.sway_socket)],
        ...
        imported = subprocess.run(
            [sys.executable, '-c',
             'import sys; import superhold.service; print("gi" in sys.modules)'],
            env=env, text=True, capture_output=True, timeout=3)
