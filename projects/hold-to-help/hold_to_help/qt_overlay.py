...
def _default_layer_library(module_file=None):
    """Resolve an installed share/hold-to-help tree, including DESTDIR staging."""
    module = Path(module_file or __file__).resolve()
    candidates = []
    if (module.parent.name == 'hold_to_help'
            and module.parent.parent.name == 'hold-to-help'
            and module.parent.parent.parent.name == 'share'):
        prefix = module.parents[3]
        candidates.extend(prefix / directory / 'hold-to-help/libhold-to-help-layer-shell.so'
                          for directory in ('lib', 'lib64'))
    candidates.append(Path('/usr/lib/hold-to-help/libhold-to-help-layer-shell.so'))
...
def _load_layer_bridge():
    ...
    path = os.environ.get('HOLD_TO_HELP_LAYER_SHELL_LIBRARY') or _default_layer_library()
    try:
        bridge = ctypes.CDLL(os.fspath(Path(path)))
        bridge.hth_layer_shell_prepare.argtypes = []
        bridge.hth_layer_shell_prepare.restype = ctypes.c_int
        bridge.hth_layer_shell_configure.argtypes = [
            ctypes.c_void_p, ctypes.c_void_p, ctypes.c_int, ctypes.c_int, ctypes.c_int]
        bridge.hth_layer_shell_configure.restype = ctypes.c_int
...
def create_application(argv=None):
    ...
    if wayland and _load_layer_bridge().hth_layer_shell_prepare() != 0:
        raise RuntimeError('could not prepare the Wayland layer-shell integration')
    _application = QApplication(arguments)
    _application.setApplicationName('hold-to-help')
    _application.setDesktopFileName('io.github.spaceghost.HoldToHelp')
...
class QtShortcutOverlay:
    ...
    def __init__(self, trigger_label='Super'):
        ...
        self.window.setObjectName('hold-to-help')
    ...
    def _place(self, context, loading=False):
        ...
        if self.wayland:
            ...
            result = self.bridge.hth_layer_shell_configure(
