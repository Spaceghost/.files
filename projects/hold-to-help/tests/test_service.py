...
try:
    from hold_to_help.service import (AlreadyRunning, GraphicalSessionGuard,
...
class RuntimeSafetyTests(unittest.TestCase):
    ...
    def test_nonprivate_runtime_is_rejected_before_state_is_created(self):
        ...
        with self.assertRaisesRegex(RuntimeError, 'owned private directory'):
            lease.acquire()
        self.assertFalse((self.runtime / 'hold-to-help').exists())
    ...
    def test_status_command_uses_package_imports_without_loading_gtk(self):
        ...
        result = subprocess.run(
            [sys.executable, '-m', 'hold_to_help', 'status', '--socket', str(self.sway_socket)],
        ...
        imported = subprocess.run(
            [sys.executable, '-c',
             'import sys; import hold_to_help.service; print("gi" in sys.modules)'],
            env=env, text=True, capture_output=True, timeout=3)
