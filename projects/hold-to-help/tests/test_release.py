...
class ReleaseTests(unittest.TestCase):
    def setUp(self):
        ...
        for name in ("bin", "data", "examples", "hold_to_help", "native", "tests", "tools"):
            (self.source / name).mkdir()
        (self.source / "hold_to_help/__init__.py").write_text("VERSION = '0.1.0'\n")
        (self.source / "SOURCE_MANIFEST").write_text("\n".join((*release.REQUIRED, "hold_to_help/__init__.py")) + "\n")
    ...
    def test_secret_and_cache_files_are_excluded(self):
        ...
        cache = self.source / "hold_to_help/__pycache__"
        cache.mkdir()
