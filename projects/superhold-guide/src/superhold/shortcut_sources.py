...
``$XDG_CONFIG_HOME/superhold/shortcuts.json`` (defaulting to
``~/.config/superhold/shortcuts.json``). Its schema is::
...
class ShortcutProvider:
    ...
    def __init__(self, socket_path, profiles_path=None):
        ...
        if profiles_path is None:
            ...
            profiles_path = config_home / 'superhold/shortcuts.json'
        ...
        for section in sections:
            for row in section['rows']:
                if 'actions' not in row:
                    row['actions'] = prepare_actions(row['key'])
                if not row['actions'] and 'unavailable_reason' not in row:
                    row['unavailable_reason'] = (
                        'Already showing guide' if row['key'] == 'Super (hold)'
                        else 'This gesture cannot be replayed as keyboard input')
        return {'app': app, 'output': output, 'sections': sections,
                'target': capture_target(window, self.socket_path)}
    ...
    def _parse_binding(cls, tokens, mode):
        ...
        reason = None
        if bind_type == 'bindcode' or flags['to-code']:
            reason = 'Physical keycode bindings require the original keyboard map'
        elif flags['input-device']:
            reason = 'This binding is restricted to a physical input device'
        elif re.search(r'(?:^|\+)Group[0-9]+(?:\+|$)', key):
            reason = 'This binding requires a specific keyboard layout group'
        elif re.search(r'(?:^|\+)button[0-9]+(?:\+|$)', key, re.IGNORECASE):
            reason = 'Pointer bindings cannot be replayed as keyboard input'
        row['actions'] = [] if reason else sway_actions(key, release=flags['release'])
        for action in row['actions']:
            action['label'] = display_key
        if reason:
    ...
    def _binding_key_identity(key, bind_type):
        ...
        for part in key.split('+'):
            ...
            if modifier:
                ...
                keys.append(sway_keysym(part) or part)
    ...
    def _tmux_key(key):
        modifiers = []
        while len(key) > 2 and key[:2] in {'C-', 'M-', 'S-'}:
            modifier = {'C-': 'Ctrl', 'M-': 'Alt', 'S-': 'Shift'}[key[:2]]
            if modifier not in modifiers:
                modifiers.append(modifier)
            key = key[2:]
        replacements = {'BSpace': 'BackSpace', 'BTab': 'Tab',
                        'PPage': 'PageUp', 'NPage': 'PageDown',
                        'DC': 'Delete', 'IC': 'Insert'}
        if key == 'BTab' or (len(key) == 1 and key.isupper()):
            if 'Shift' not in modifiers:
                modifiers.append('Shift')
        key = replacements.get(key, key)
        if modifiers and len(key) == 1 and key.isalpha():
            key = key.upper()
        return '+'.join([*modifiers, _clean(key, 40)])
