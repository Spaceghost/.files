...
class HoldState:
    ...
    def pressed_codes(self):
        return frozenset(code for codes in self._pressed.values() for code in codes)
...
class EvdevMonitor:
    ...
    def _read_device(self, path, device, now, budget):
        ...
        while consumed < budget and calls < budget:
            ...
            try:
                ...
                device.drained = not device.buffer and not device.dropped
                break
    ...
    def poll(self, now):
        ...
        self.input_synced = (bool(self._devices)
                             and all(device.drained for device in self._devices.values()))
        if self._devices and not self.input_synced:
            # Event/call budgets, partial records and SYN_DROPPED can all leave
            # a key unread. Cancel holds/actions until every keyboard reaches
            # EAGAIN with no partial record or outstanding resynchronization.
            self.state.cancel()
