...
LIB = Path(__file__).resolve().parents[1] / 'src'
sys.path.insert(0, str(LIB))

from superhold.config import SectionLayout
...
class FixtureProvider(ShortcutProvider):
    ...
    def snapshot(self):
        # These pre-existing discovery tests compare the presentation contract.
        # Dedicated action tests below inspect the complete production snapshot.
...
'''
    ...
    def test_modifier_order_and_keysym_case_are_normalized_for_effective_overrides(self):
        config = '''
...
'''
        ...
        self.assertEqual(descriptions, ['exec uppercase-keysym'])

    def test_case_insensitive_sway_unbind_removes_named_symbol_but_not_numeric_uppercase(self):
        config = 'bindsym Mod4+Shift+x exec first\nbindsym Mod4+Shift+0x58 exec numeric\nunbindsym Shift+Mod4+X\n'
...
'''
    ...
    def test_configured_order_hidden_and_titles_rearrange_the_guide(self):
        layout = SectionLayout.from_document({
            'order': ['system', 'desktop'], 'hidden': ['application'],
            'titles': {'desktop': 'Window manager'}})
        snapshot = FixtureProvider(self.replies(), sections=layout).snapshot()
    ...
    def test_custom_section_is_ordered_and_renamed_like_a_built_in(self):
        layout = SectionLayout.from_document({
            'order': ['mine', 'desktop'], 'titles': {'mine': 'Muscle memory'},
            'custom': {'mine': {'title': 'My keys', 'coverage': 'Partial local section',
                                'rows': [{'key': 'Super+G',
                                          'description': 'Grid overlay'}]}}})
        snapshot = FixtureProvider(self.replies(), sections=layout).snapshot()

        self.assertEqual(snapshot['sections'][0]['title'], 'Muscle memory')
        self.assertEqual(snapshot['sections'][0]['coverage'], 'Partial local section')
        self.assertEqual(snapshot['sections'][0]['rows'][0]['key'], 'Super+G')
        # Unlisted sections keep their default place after the listed ones.
        self.assertEqual([section['title'] for section in snapshot['sections']],
                         ['Muscle memory', 'Sway — default', 'Firefox shortcuts',
                          'System controls'])

    def test_unlisted_sections_keep_the_default_local_first_order(self):
        layout = SectionLayout.from_document({'order': ['system']})
        snapshot = FixtureProvider(self.replies(), sections=layout).snapshot()
    ...
    def test_malformed_custom_profile_is_reported_without_hiding_builtin_profiles(self):
        with tempfile.TemporaryDirectory(prefix='superhold-test-') as temp:
...
class ShortcutActionMetadataTests(unittest.TestCase):
    def provider(self, config='', app='firefox'):
        return FixtureProvider({
            ShortcutProvider.GET_TREE: focused_tree(app, pid=os.getpid()),
            ShortcutProvider.GET_CONFIG: {'config': config},
            ShortcutProvider.GET_BINDING_STATE: {'name': 'default'},
            ShortcutProvider.GET_VERSION: {'loaded_config_file_name': '/tmp/config'},
        })

    def test_snapshot_attaches_individual_profile_choices_and_private_target_identity(self):
        snapshot = ShortcutProvider.snapshot(self.provider())
        row = next(row for row in snapshot['sections'][0]['rows'] if row['key'] == 'Alt+Left / Alt+Right')
        self.assertIn('actions', row)
        self.assertEqual(row['actions'], [
            {'label': 'Alt+Left', 'sequence': [{'key': 'Left', 'modifiers': ['alt']}]},
            {'label': 'Alt+Right', 'sequence': [{'key': 'Right', 'modifiers': ['alt']}]},
        ])
        self.assertEqual(snapshot['target']['con_id'], 4)
        self.assertEqual(snapshot['target']['pid'], os.getpid())
        self.assertTrue(snapshot['target']['process_start'])
        self.assertNotIn('name', snapshot['target'])

    def test_sway_raw_keys_are_preserved_and_restricted_bindings_are_explained(self):
        snapshot = ShortcutProvider.snapshot(self.provider('''
bindsym Mod4+x exec any-untrusted-command
bindsym --release Mod4+Shift+X exec other
bindsym --input-device=physical Mod4+y exec another
bindsym Group2+Mod4+z exec another
bindcode Mod4+42 exec another
bindsym button1 exec another
'''))
        rows = snapshot['sections'][1]['rows']
        self.assertIn('actions', rows[0])
        self.assertEqual(rows[0]['actions'], [{'label': 'Super+X', 'sequence': [
            {'key': 'x', 'modifiers': ['logo']}]}])
        self.assertEqual(rows[1]['actions'][0]['sequence'], [
            {'key': 'x', 'modifiers': ['logo', 'shift']}])
        for row in rows[2:]:
            self.assertEqual(row['actions'], [])
            self.assertTrue(row['unavailable_reason'])
        self.assertNotIn('exec', str(rows[0]['actions']))

    def test_neovim_commands_submit_and_all_builtin_keyboard_alternatives_are_executable(self):
        from superhold.shortcut_profiles import PROFILES
        for profile in PROFILES:
            snapshot = ShortcutProvider.snapshot(self.provider(app=profile))
            for row in snapshot['sections'][0]['rows']:
                with self.subTest(profile=profile, key=row['key']):
                    self.assertTrue(row.get('actions'))
        nvim = ShortcutProvider.snapshot(self.provider(app='nvim'))
        write = next(row for row in nvim['sections'][0]['rows'] if row['description'] == 'Write file')
        self.assertEqual(write['key'], ':w, Enter')
        self.assertEqual(write['actions'][0]['sequence'], [
            {'text': ':w'}, {'key': 'Return', 'modifiers': []}])
