...
MODULE = (Path(__file__).resolve().parents[1]
          / 'src/superhold/shortcut_hold.py')
...
class EvdevMonitorTests(unittest.TestCase):
    ...
    def test_interrupted_read_budget_never_claims_pending_input_is_synchronized(self):
        fd = self.add_device()
        monitor = self.monitor(max_events_per_poll=4)
        ...
        self.backend.read = interrupted
        cancel_serial = monitor.state.cancel_serial
        monitor.poll(.1)
        self.assertFalse(monitor.input_synced)
        self.assertGreater(monitor.state.cancel_serial, cancel_serial)
        self.backend.read = original_read
        monitor.poll(.2)
        self.assertTrue(monitor.input_synced)
        self.assertEqual(monitor.state.pressed_codes, frozenset({30}))

    def test_partial_record_is_unsynchronized_until_complete_and_drained(self):
        fd = self.add_device()
        monitor = self.monitor()
        self.addCleanup(monitor.close)
        monitor.poll(0)
        event = packet(1, 30, 1)
        self.backend.reads[fd] = [event[:5]]
        monitor.poll(.1)
        self.assertFalse(monitor.input_synced)
        self.backend.reads[fd] = [event[5:]]
        monitor.poll(.2)
    ...
    def test_every_keyboard_must_be_drained_in_current_poll(self):
        first = self.add_device('/dev/input/event1', 11)
        second = self.add_device('/dev/input/event2', 12)
        monitor = self.monitor(max_events_per_poll=4)
        self.addCleanup(monitor.close)
        monitor.poll(0)
        self.assertTrue(monitor.input_synced)
        # Rotating fairness visits the second keyboard first on this poll.
        self.backend.reads[second] = [packet(1, 125, 2) * 4]
        self.backend.reads[first] = [packet(1, 30, 1)]
        monitor.poll(.1)
        self.assertFalse(monitor.input_synced)
        monitor.poll(.2)
        self.assertTrue(monitor.input_synced)
