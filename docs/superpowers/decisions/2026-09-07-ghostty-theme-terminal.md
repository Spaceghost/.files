...
Super+Enter launches Ghostty. The binding uses the desktop's `$term` variable;
ordinary theme profile changes do not replace the shortcut configuration.
...
To restore the previous shortcut, set `$term` to `foot` in the Sway config and
