...
`mbp-intel-wallpaper new-theme`. Both left and right modifier keys work, including
...
The signed `mbp-intel-waybar-art-1.0.0-r4.apk` built twice byte-identically without
...
with `doas apk --no-network add --allow-downgrade /path/to/mbp-intel-waybar-art-1.0.0-r3.apk`,
