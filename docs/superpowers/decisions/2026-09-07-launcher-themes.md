...
Every Oldbook Fuzzel entry point uses `oldbook-fuzzel`, which reads
...
integrations should invoke `~/.local/bin/oldbook-fuzzel`.
...
ShellCheck accepts oldbook-menu. Actual isolated Sway renders are in
...
oldbook-wallpaper with ongoing desktop-panel work; its two wrapper substitutions
...
and remove the new oldbook-fuzzel HOME symlink. Reopen menus; no compositor restart
